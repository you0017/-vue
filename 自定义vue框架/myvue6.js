...
class Compile {
			...
			if (this.isElement(node)) {
				...
				Array.from(nodeAttrs).forEach(attr => {
					...
					if (this.isEvent(attrName)) {
						...
						this.addHandle(node,eventType,this.$vm,attrValue);
					}
				})
			}
	...
	//判断是否为vue支持的指令  这类指令以q-开头
	isDirective(attrName) {
		return attrName.indexOf("q-") == 0;
	}
	//判断是否为事件的指令  @开头+事件名字  例  @click="show"
	isEvent(attrName) {
	...
	model(node,vm,key){
		...
		//对node增加一个事件input,只要页面上的input值改变，对应的vm[key]就要改
	 ...
	 * 事件绑定
	 */
	addHandle(node,eventType,vm,methodName){
